...
public class RouteGuideServer {
     ...
     */
    private RouteGuideServer(int port, URL featuresFile) {
        try {
            this.port = port;
            features = RouteGuideUtil.parseFeatures(featuresFile);
        } catch (IOException e) {
    ...
    public void start() throws IOException {
        server = ServerBuilder.forPort(port).addService(RouteGuideGrpc.bindService(new RouteGuideService(features)))
                .build().start();
    ...
    private static class RouteGuideService implements RouteGuideGrpc.RouteGuide {
        private final Collection<Feature> features;
        private final ConcurrentMap<Point, List<RouteNote>> routeNotes = new ConcurrentHashMap<Point,
                List<RouteNote>>();
        ...
        private Feature checkFeature(Point location) {
            for (Feature feature : features) {
                if (feature.getLocation().getLatitude() == location.getLatitude() && feature.getLocation()
